...
- Requires running **MongoDB** and **Redis** instances, plus a `.env` file (copy of `.env.example`): `NODE_ENV`, `DOMAIN`, `PORT`, `SEFINEK_API` (base URL of the elevation API, exposed to the frontend), `SITE_MODE` (`auto`/`poland`/`global` - forces a site regardless of host), `MONGODB_URL`, `STATS_SNAPSHOT_SECRET` (see `routes/Internal.js`), `REDIS_HOST`, `REDIS_PASSWD`, optionally `MAPTILER_API_KEY` and `CARTO_API_KEY` (unlock extra basemaps).

## Architecture

### Backend (Express, CommonJS)

- `index.js` - entry point. Loads `.env` (`process.loadEnvFile()`), immediately starts `startNodesRefreshJob()` (`services/nodes.js`) and imports `global/database/mongoose.js` (side effect, before the server starts listening). Middleware order: helmet → `express.static('public')` → site resolver (`req.site` from `config/sites.js` based on `req.hostname`) → language-prefix detection in the URL → i18next (`middlewares/language.js`) → language negotiation/redirects (`middlewares/languageNegotiation.js`) → morgan → rate limiter (production only) → timeout. `app.locals` exposes only `domain` and `v` (version from `package.json`) to EJS views - `sefinekApi`/`cartoApiKey`/`maptilerApiKey` are read directly from `process.env` in `views/index.ejs`.
...
- `routes/Internal.js` - `POST /api/internal/stats-snapshot`, guarded by a shared secret (`STATS_SNAPSHOT_SECRET`) compared via `crypto.timingSafeEqual` against the `X-Internal-Secret` header. Computes `getStats()` for both regions and upserts a `StatsDaily` row each. Not called by anything in this app - triggered once a day by the `meshcore-profiles/cronjobs` worker (a separate repo/process), which owns the *scheduling* only; this repo keeps the DB write next to its Mongoose connection and its already-computed in-process node stats, so nothing about the node-fetching/stats pipeline is duplicated elsewhere.
...
- `global/database/models` - the `StatsDaily` model (unique index on `{ region, date }`) for historical stats.
