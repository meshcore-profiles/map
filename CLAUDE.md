...
- `global/database/syncIndexes.js` - standalone script run manually (`node global/database/syncIndexes.js`), syncs Mongoose indexes from every model in this repo's own `global/database/models` against the actual state in MongoDB.
...
- `services/nodes.js` - the node data layer, now a pure Redis reader. Fetching the full node set from upstream (`map.meshcore.io`) and computing the Poland-only subset (point-in-polygon against a simplified Poland border, with an upfront bounding-box filter) happens entirely in the separate `meshcore-profiles/cronjobs` repo (`jobs/nodes-refresh.js`, own `utils/geo.js` + `jobs/data/poland-border.geojson`), which runs every 10 minutes and writes packed msgpack buffers straight into Redis (`nodes:all`, `nodes:pl`, plus an ISO timestamp at `nodes:updatedAt`) - same Redis instance/database this app already uses via `global/services/redis.js`.
  - `getCachedNodes(region)` reads the packed buffer for a region straight from Redis (a typed client mapping the reply to a `Buffer`).
  - `getLastRefreshedAt()` reads `nodes:updatedAt` from Redis (async - no in-process refresh event exists here anymore).
  - `getStats()` computes and memoizes repeater/type/status statistics for a given region's node set; the memoized cache is invalidated whenever `nodes:updatedAt` has moved on since it was last memoized, not by any local refresh.
...
- `global/database/models` - the `StatsDaily` model (unique index on `{ region, date }`) for historical stats. This app only reads it (`routes/Api.js`'s `/api/v1/stats/history`); the daily snapshot rows are written by the `meshcore-profiles/cronjobs` worker directly (`jobs/nodes-stats-snapshot.js`), through its own copy of this same shared model.
...
- `global/` - git submodule ([meshcore-profiles/global](https://github.com/meshcore-profiles/global)), shared verbatim across every MeshCore repo in this org (not just this one) - **never edit files under `global/` from inside this repo**; change them in the `global` repo itself, commit/push there, then bump the submodule pointer here (`cd global && git pull && cd .. && git add global && git commit`).
  - `global/services/redis.js` - a single shared Redis client (database 8), connected once on import.
  - `global/services/axios.js` - a shared axios instance with a custom `User-Agent` derived from **this repo's own** `package.json` (resolved via a fixed `../../package.json` from the submodule's location, not the submodule's own) and a 14s timeout; used by `services/nodes.js` and `global/IndexNow.js`.
  ...
  - `global/database/syncIndexes.js` - see Commands above; resolves this repo's `global/database/models` the same way (fixed relative path from the submodule's location), not a models directory inside the submodule itself (models are app-specific, never shared).
